...
void AutoControl( int player )
{
	...
	{				
		...
		{
			case e_AutoCommand_Message:
				StartBalloon( autoPattern->message );
				autoPattern++;
				break;
				
			case e_AutoCommand_Wait:
				if( !tutorialTime )
				{
			...
			case e_AutoCommand_BlockUntilLand:
				if( role[player] != kFalling ) 
				{
					autoPattern++;
				}			
			...
			case e_AutoCommand_kBlockUntilDrop:
			...
			case e_AutoCommand_ShowStartMenu:
				EndTutorial( );
				break;
			
            case e_AutoCommand_BlockUntilComplete:
                if( role[player] == kWaitForRetrieval )
                {
                    autoPattern++;
                }			
            ...
            case e_AutoCommand_UpdateBackdrop:
            {
                CC_RGBSurface* overlaySurface = LoadPICTAsRGBSurface(backgroundID, autoPattern->message);
                if (overlaySurface != nullptr)
                {
                    MRect          overlayRect    = { 0, 0, short(overlaySurface->h), short(overlaySurface->w) };
                    
                    SurfaceBlendOver(backdropSurface, &overlayRect, overlaySurface, &overlayRect);
                    SurfaceBlendOver(g_frontSurface,  &overlayRect, overlaySurface, &overlayRect);
                    
                    Gfx_FreeSurface(overlaySurface);
                }
...
void AIControl( int player )
{
	...
	if( destinationX[player] == blobX[player] &&
        ...
        RandomBefore( 100 ) < std::min<int>(15, character[player].intellect ))
	{
		DoDrop( player );
	}
}
