...
void PlaceGrenade( int player )
{
    ...
    {
        // Keep track of the max bomb amount.
        if (multiplier > g_allTimeBiggestBomb)
        {
            // If we just exceeded 12, award the achievement.
            if (g_allTimeBiggestBomb < 12 && multiplier >= 12)
            {
                Platform_ReportAchievement("BombSquad");
            }
            
            g_allTimeBiggestBomb = multiplier;
        }
    }
...
int ScoreToReachSolitaireLevel(int level)
{
    static const int levelClear[] =
    {
        0,
        2500,
        7000,
...
void RetrieveBlobs( int player )
{
    ...
    if (g_dropLockEnabled)
    {
        if (players == 1)
        {
            // In one player mode, the player could be using either controller.
            // If player 0 is retrieving blobs, clear both drop locks.
            // If player 1 is retrieving blobs, that's the CPU. Do nothing.
            if (player == 0)
            {
                hitKey[0].drop = 0;
                hitKey[1].drop = 0;
        ...
        {
            // In two player mode, the controllers are bound to the player.
	...
	{
		if ((level <= kLevels) && (score[player] >= ScoreToReachSolitaireLevel(level)))
		{
			PlayMono( kLevelUp );
            ...
            if (level > kLevels)
            {
                UI_ShowWorld3VictoryScreen(e_AfterUIScreen_Title);
            }
            else
            {
                if (!InitCharacter(1, level))
                {
                    // This shouldn't happen--we go to victory in Win() now.
                    UI_AddHighScoreScreen(score[0], e_AfterUIScreen_GameOver);
                    return;
                }

                ChooseMusic(-1);
                UI_ShowPuppetShowForLevel(level, e_AfterPuppetShow_ContinueSolitaireGame);
...
{
    playerWindowZRect = { 0, 0, kBlobVertSize * (kGridDown - 1), kBlobHorizSize * kGridAcross };
