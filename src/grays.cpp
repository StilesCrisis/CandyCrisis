        ...
        if( rowBounce[player][x] == -1 )
        {
            ...
            {			
                    ...
                    (suction[player][x][y] == kGrayNoBlink) )
                {
                    ...
                    SurfaceDrawSprite( &myRect, kGray, currentBlinkGraphic );
...
void SetupGrays( int player )
{
	...
	for( grayX=0; grayX < kGridAcross; grayX++ )
	{
		...
		while( grid[player][grayX][rowFree[grayX]+1] == kEmpty && rowFree[grayX] < (kGridDown-1) )
			rowFree[grayX]++;
	}
	...
	if( lockGrays[player] > 0 )
	{
		...
		while( (onlyOnce[0] || onlyOnce[1] || onlyOnce[2] || onlyOnce[3] || onlyOnce[4] || onlyOnce[5]) && (lockGrays[player] > 0) )
		{
...
void Bounce( int player )
{
    ...
    for( x=0; x<kGridAcross; x++ )
    {
        ...
        if( bounce >= 0 )
        {
            float compressAmount = (-0.148187 * bounce + 2.3722) * bounce + 2.49464; // 3-scale
            float compress = std::max<>(0.0f, compressAmount) - kBlobVertSize;
            ...
            float blobTop = kBlobVertSize * (kGridDown-1);
            
            for( y=kGridDown-1; y>=rows; y-- )
            {
                suck = suction[player][x][y];
                blob = grid[player][x][y];
                if( suck == kNoSuction && blob >= kFirstBlob && blob <= kLastBlob && compressAmount > 3*(3))
                {
                    suck = kSquish;
                }
                
                if( blob == kGray )	SurfaceDrawSprite( &blobRect, kGray, currentBlinkGraphic );
